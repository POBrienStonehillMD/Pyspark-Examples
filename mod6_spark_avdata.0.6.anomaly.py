...
df=df.withColumn('Date', fun.to_date("timestamp"))
df=df.withColumn('days_ago', fun.datediff(fun.current_date(), "timestamp"))
df=df.withColumn('days_ago', fun.datediff(fun.current_date(), "timestamp"))

# Calculate week over week change (7 day window change)
windowSpec  = Window.partitionBy("ticker").orderBy("days_ago")
dflag = df.withColumn("lag",fun.lag("open",7).over(windowSpec))
dflag = dflag.withColumn('oneweekdiff', fun.col('lag') - fun.col('open'))
dflag.show()

# Within group (ticker) calculate anomaly time periods
# filter for anomalous events (keep outliers)
statsDF = dflag.groupBy("ticker").agg(fun.mean(dflag.oneweekdiff).alias("mean"), fun.stddev(dflag.oneweekdiff).alias("stddev"))
...
#MEME Stock Filter
statsDF = statsDF[statsDF["statsticker"]=="GME"]
statsDF.show()
...
# I decided to update the code to a one week diff and to filter for a popular MEME stock under the past year. 
